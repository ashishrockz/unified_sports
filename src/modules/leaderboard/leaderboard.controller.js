...
const cricketBattingHandler = async (req, res, next) => {
  try {
    ...
    const data = await getCricketBattingLeaderboard({ period, limit });
...
const cricketBowlingHandler = async (req, res, next) => {
  try {
    const { period, limit } = req.query;
    const data = await getCricketBowlingLeaderboard({ period, limit });
...
const winsHandler = async (req, res, next) => {
  try {
    const { sport, period, limit } = req.query;
    const data = await getWinsLeaderboard({ sport, period, limit });
...
const mostMatchesHandler = async (req, res, next) => {
  try {
    const { sport, period, limit } = req.query;
    const data = await getMostMatchesLeaderboard({ sport, period, limit });
